...
def update_output_container(selected_statistic, selected_year):
 if selected_statistic == 'Yearly Statistics' and selected_year != 'Select-year':
        filtered_data = df[df['Year'] == selected_year]
        fig = px.line(filtered_data, x='Year', y='Statistic1', title='Yearly Statistics')
        return dcc.Graph(figure=fig)
 elif selected_statistic == 'Recession Period Statistics':
        recession_data = data[data['Recession'] == 1]
        fig = px.bar(recession_data, x='Year', y='Statistic2', title='Recession Period Statistics')
        return dcc.Graph(figure=fig)
        return "Please select a valid statistic and year."
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        ...
        R_chart4 = dcc.Graph(
            figure=px.bar(unemp_data, x='unemployment_rate', y='Automobile_Sales', color='Vehicle_Type',
                      title="Unemployment Rate vs Vehicle Type Sales")
    )
...
# Plot 2 Total Monthly Automobile sales using line chart.
  ...
  # Plot bar chart for average number of vehicles sold during the given year
         # grouping data for plotting.
         # Hint:Use the columns Year and Automobile_Sales
        avr_vdata=yearly_data.groupby('Years')['Automobile_Sales'].mean().reset_index()
